...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const profileRes = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/profile', { headers }); const mailbox = await profileRes.json(); if (!profileRes.ok) throw new Error(mailbox.error?.message ?? 'Gmail profile 讀取失敗');
    ...
    if (useFull) { let token = ''; for (let page = 0; page < 10; page += 1) { const url = new URL('https://gmail.googleapis.com/gmail/v1/users/me/messages'); url.searchParams.set('maxResults', '500'); if (token) url.searchParams.set('pageToken', token); const response = await fetch(url, { headers }); const list = await response.json(); if (!response.ok) throw new Error(list.error?.message ?? 'Gmail message list 讀取失敗'); messageIds.push(...(list.messages ?? []).map((item: { id: string }) => item.id)); token = list.nextPageToken ?? ''; if (!token) break; } }
    let synced = 0;
    for (const id of [...new Set(messageIds)]) { const response = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}?format=full`, { headers }); const message = await response.json(); if (!response.ok) continue; const h = Object.fromEntries((message.payload?.headers ?? []).map((item: { name: string; value: string }) => [item.name.toLowerCase(), item.value])); const parsed = collect(message.payload ?? {}); const from = address(h.from); const to = address(h.to); const outbound = from === String(mailbox.emailAddress).toLowerCase() || (message.labelIds ?? []).includes('SENT'); const counterpart = outbound ? to : from; if (!counterpart) continue;
  ...
  } catch (err) { const message = err instanceof Error ? err.message : String(err); await admin.from('audit_log').insert({ action: 'gmail_sync', result: 'error', detail: message }); return jsonResponse({ error: message }, 500); }
});
