 ...
 *
 * `variant="collapsed"`（WP9）：同一段自我介紹在首頁與四個服務頁各出現一次全文，
 * 讀者翻到第二頁之後看到的都是重複內容。服務頁改成「兩行引言＋展開全文」，一個字都
 * 沒刪，只是不再預設攤開。首頁維持完整版——那裡是第一次見面，該完整講。預設 variant
 * 仍是 'full'，既有呼叫端不改也不會變。
 */
...
export interface AboutFounderProps {
  /** 'collapsed'：只常駐兩行引言，全文收進「認識發起人」展開鈕（各寬度皆然）。 */
  variant?: 'full' | 'collapsed';
}

export function AboutFounder({ variant = 'full' }: AboutFounderProps) {
  const body = (
    <>
    ...
    <section className="max-w-4xl mx-auto px-4 mt-20 mb-12">
      <div className="bg-cream border-2 border-brown rounded-3xl p-6 md:p-10 shadow-warm relative">
        ...
        {variant === 'collapsed' ? (
          <div className="mt-4">
            <CollapsibleSection
              expandedOnDesktop={false}
              label="認識發起人"
              collapseLabel="收合自我介紹"
              summary={
                <div className="bg-white/70 p-5 rounded-2xl border-2 border-brown/10 backdrop-blur-sm shadow-sm font-body text-brown leading-relaxed font-medium text-justify space-y-2">
                  <p className="text-lg">我從小學三年級就診斷ADHD，一路跌跌撞撞成長到大。</p>
                  <p className="text-xl font-bold text-[#006064]">我希望可以幫助更多像我一樣的人！</p>
                </div>
              }
            >
              {body}
          ...
          <div className="space-y-8 mt-4">{body}</div>
        )}
      </div>
    </section>
  );
