...
## 23. 2026-08-06 兒少姓名去識別化缺口、刪報名不回收名額

migration 44 支全對齊、未套用 0（`20260806000032` 已套用）。`generate-document` 已重新部署。

### P1 ⚠ Phase 6 的去識別化漏掉孩子姓名

`generate-document` 代號化了報名者姓名，卻把 `answers.childName` 原樣送進 prompt。監督視窗在三次預覽都看到，最近一次是群發驗收：

```
報名：〔家長2〕（confirmed）／孩子：測試童
```

家長受保護、孩子沒有。**報名表那一欄的說明是「孩子姓名或代號」，實務上家長多半填真名**，所以正式資料裡那個值就是兒少的真實姓名——在一條「姓名固定去識別化」的裁決底下送給外部 API。

修法：孩子獨立一組 `〔孩子N〕`，來源含舊的平面 key `childName`、新群組的 `children[].alias`，外加任何看起來像姓名欄位的鍵（舊報名沒有固定 schema，只列舉白名單會漏掉下一版表單新增的欄位）。因為是**先收集姓名、再對整段素材替換**，填在自由填答（議題欄）裡的同一個名字也會一併換掉。

**孩子代號不還原。** 家長姓名還原是因為草稿會變成寄給那位家長的信、需要稱呼；孩子沒有這個需要，還原只是把兒少姓名重新寫回產出物。兩份對照表因此刻意不對稱（孩子只進 `placeholders`、不進 `restore`），守門有一條斷言擋住它被改回去。

**驗證方式**：把真正的去識別化函式抽出來、對監督視窗回報的那個字串實跑——沒有任何真實字串外洩、家長還原、孩子維持代號。並對「修正前的行為」跑同一個測試，仍然吐出 `測試童`，證明這個測試分辨得出差異而不是恰好都通過。

### P2 直接刪報名不會回收 `booked_count`

`enforce_session_capacity` 只在 INSERT 遞增，名額回收綁在狀態轉換函式上，**DELETE 沒有任何對應**。正常流程踩不到（後台都走轉換函式），但任何繞過 UI 的資料清理都會留下錯誤的名額計數——而那個數字直接決定前台顯示的剩餘名額。清測試資料時實證：報名刪了、場次 `booked_count` 仍停在 2。

`20260806000032` 補 after delete trigger。⚠ **必須看 `capacity_released_at`**：已退回／取消的報名早就把名額還回去了，刪除時再扣一次會吃掉別人的名額且沒有錯誤訊息——`20260804000015` 修過同一類問題（移轉場次時對已釋額報名重複扣，症狀是「別的場次名額悄悄少一個」）。

DEPLOY.md 加了資料清理段落與對帳查詢。查證正式庫現況：22 個開放場次，無負數、無超額。
