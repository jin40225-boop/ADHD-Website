...
### 待決：同儕報名表欄位對齊（migration `20260804000011`，**尚未套用**）

`/peer-group/register` 目前只有 3 個欄位（nickname／email／note），02_v2 定稿有 5 個。差異：缺「手機號碼」（選填）、缺「是否第一次參加？」（選填單選，給主持人備場用）；`note` 的標籤是「想對彥宇說的話（選填）」，定稿為「想聊或想問的話題」且用途寫明「讓客座夥伴可以先準備」。migration 另把定稿的提示語「表單是預先統計人數用的！沒報名也可以當天直接參加。」寫進 `projects.description`。

02_v2 把報名做成服務頁上的彈窗，實作是獨立的 `/peer-group/register` 路由——這是重構刻意的架構差異（表單一律由 `form_schemas` 驅動，Phase 3 後台才能編輯），不是落差。

### Phase 2 剩餘項目

無（待 `20260804000011` 核可套用後即完成）。
